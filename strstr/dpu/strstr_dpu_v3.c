...
__host uint32_t RECORDS_LENGTH=0;
...
void writeBackRecords(uint8_t* record_start, uint32_t length) {
    ...
    mram_write(record_start-(uintptr_t)record_start%8, &RECORDS_BUFFER[temp], roundUp8(length+1));
    
    if((uintptr_t)record_start%8 != 0) {
        printf("write back wram address not 8 byte aligned\n");
    }

    if((uintptr_t)&RECORDS_BUFFER[temp]%8 !=0) {
        printf("write back mram address not 8 byte aligned\n");
    }
    printf("mram address %x\n", (uintptr_t)(&RECORDS_BUFFER[temp]));
