...
int host_str(char* json_str, uint32_t len) {
  char*s1,*s2;
  char*cp = json_str;
...
void host_strstr(unsigned char *f) {
    unsigned char * start = f;
    unsigned char *  end;
    unsigned char *max = &(f[BUFFER_SIZE]);
    //char key[] ="aabaa\n";
    printf("start %ld max %ld\n", ((uintptr_t)start), (uintptr_t)max);
    while(start < max) {
        
        end =  (unsigned char*)memchr(start, '\n', BUFFER_SIZE);

        if(host_str((char*)start,end-start)) {
            printf("host found strstr\n");
        }
