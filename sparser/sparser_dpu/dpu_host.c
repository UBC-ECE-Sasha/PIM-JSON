...
void multi_dpu_test(char *input, unsigned int * keys, uint32_t keys_length, long length, struct json_candidate record_offsets[NR_DPUS][MAX_NUM_RETURNS], uint64_t input_offset[NR_DPUS][NR_TASKLETS], uint32_t output_count[NR_DPUS]){
    ...
    DPU_RANK_FOREACH(set, dpu_rank, rank_id)
    {
        ...
        DPU_FOREACH(dpu_rank, dpu)
        {
            ...
            DPU_ASSERT(dpu_copy_to_mram(dpu.dpu, dpu_mram_buffer_start, (unsigned char*)input+input_offset[dpu_id][0], input_length[dpu_id]));
#endif
...
#endif
#ifdef NORM_TRANSFER
            DPU_ASSERT(dpu_copy_from(dpu, "candidates", 0, &(record_offsets[dpu_id]), sizeof(struct json_candidate) * ALIGN(output_count[dpu_id], 8)));
#endif
            dpu_id++;
        }
